...
<div class="page">
    <div class="left">
        <div class="range-inputs">
            ...
            <button type="button" onclick="calculateIds()">OK</button>
        </div>
...
//				console.log(rtn);
    ...
    function calculateIds() {
        ...
        if (!startId || !endId) {
            resultsDiv.innerHTML = "<p>Please enter both start and end IDs.</p>";
            return;
        }

        const startPrefix = startId.match(/[a-zA-Z]+/)[0];
        const startNum = parseInt(startId.match(/\d+/)[0], 10);
        const endPrefix = endId.match(/[a-zA-Z]+/)[0];
        const endNum = parseInt(endId.match(/\d+/)[0], 10);

        if (startPrefix !== endPrefix) {
            resultsDiv.innerHTML = "<p>Start and end IDs must have the same prefix.</p>";
            return;
        }

        if (startNum > endNum) {
            resultsDiv.innerHTML = "<p>End ID must be greater than or equal to start ID.</p>";
            return;
        }

        let ids = [];
        for (let i = startNum; i <= endNum; i++) {
            ids.push(startPrefix + i.toString().padStart(startNum.toString().length, '0'));
        }

        resultsDiv.innerHTML = "<p>Generated IDs:</p><ul>" + ids.map(id => `<li>${id}</li>`).join('') + "</ul>";
    }
